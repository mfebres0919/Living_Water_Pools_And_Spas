...
/* ============================================================
   E-POOLS — hero carousel + gallery Reels/Photos tabs
   ============================================================ */
(function () {
  'use strict';
  ...
  (function heroCarousel() {
    var slides = Array.prototype.slice.call(document.querySelectorAll('#hero .eph-slide'));
    var dots   = Array.prototype.slice.call(document.querySelectorAll('#hero .eph-dot'));
  ...
  /* ── Projects: "Instagram Reels" filter tab ──
     The reels live inside the Projects section. When the "Instagram Reels"
     filter is active we swap the project grid for the reels panel; any other
     filter swaps it back. (The photo filtering itself is handled by
     js/gallery-filter.js — this just toggles grid vs. reels visibility.) */
  (function projectsReels() {
    var wrap       = document.querySelector('#projects .projects-grid-wrap');
    var filterBtns = Array.prototype.slice.call(document.querySelectorAll('#projects .filter-btn'));
    if (!wrap || !filterBtns.length) return;

    filterBtns.forEach(function (btn) {
      btn.addEventListener('click', function () {
        wrap.classList.toggle('show-reels', btn.getAttribute('data-filter') === 'reels');
      });
    });

    // "Watch Our Reels" button in the header jumps to the reels tab.
    var watchBtn    = document.getElementById('watchReelsBtn');
    var reelsFilter = document.querySelector('#projects .filter-btn[data-filter="reels"]');
    if (watchBtn && reelsFilter) {
      watchBtn.addEventListener('click', function () { reelsFilter.click(); });
    }
  })();

  /* ── Contact hours: highlight today + Open/Closed status ──
     Open Mon–Fri 9:00 AM–5:00 PM; closed weekends. Edit OPEN_HOUR /
     CLOSE_HOUR / the data-closed attributes to change the schedule. */
  (function contactHours() {
    var list     = document.getElementById('hoursList');
    var statusEl = document.getElementById('contactStatus');
    if (!list) return;

    var OPEN_HOUR = 9, CLOSE_HOUR = 17;   // 9:00 AM – 5:00 PM
    var now  = new Date();
    ...
    var closedToday = todayRow ? todayRow.getAttribute('data-closed') === 'true' : true;
    var isOpen = !closedToday && mins >= OPEN_HOUR * 60 && mins < CLOSE_HOUR * 60;
...
/* ─────────────────────────────────────────────────────────────
   STAGGERED FADE-IN  (service detail pages)
   Each [data-fade] section reveals its child elements one-by-one when it
   scrolls into view. Only runs if [data-fade] exists, so other pages are
   untouched. Pairs with the .fade-on / .is-in CSS in css/service-detail.css.
   ───────────────────────────────────────────────────────────── */
(function () {
  'use strict';
  var groups = document.querySelectorAll('[data-fade]');
  if (!groups.length) return;

  /* Elements that fade in, in document order, inside each section. Keep this
     list identical to the one in css/service-detail.css. */
  var SEL = '.page-hero-content > h1, .page-hero-breadcrumb, ' +
            '.svc-main-img, .svc-main > h2, .svc-main > p, .svc-bullets, .svc-benefits, ' +
            '.svc-steps-head, .svc-steps-intro, .svc-step, ' +
            '.svc-nav > li, .svc-help, .svc-brochure, ' +
            '.lets-talk-eyebrow, .lets-talk h2, .lets-talk-line, .lets-talk .btn-primary';
  var STEP = 90; // ms between each element in a section
  ...
  /* No IntersectionObserver (or motion not gated on) → just show everything. */
  if (!('IntersectionObserver' in window)) {
